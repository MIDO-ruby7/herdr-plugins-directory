...
def build() -> None:
    ...
    for repo in repos:
        ...
        override_description = overrides.get(repo["full_name"], {}).get("description")
        description = override_description or (repo.get("description") or "").strip()
        entries.append({
...
UI = {
    "ja": {
        ...
        "description_note": "GitHub 上に説明文が無いリポジトリは、`description` キーで上書きできます"
                             "（英語で。ja/zh ページの訳文は `data/translations.json` に追加してください）。",
    ...
    "en": {
        ...
        "description_note": "For repos with no GitHub description, override it with a `description` key "
                             "(in English; add ja/zh translations to `data/translations.json`).",
    ...
    "zh": {
        ...
        "category_keys": "分类键：",
        "description_note": "对于 GitHub 上没有描述的仓库，可以用 `description` 键覆盖"
                             "（用英文；ja/zh 页面的译文请添加到 `data/translations.json`）。",
